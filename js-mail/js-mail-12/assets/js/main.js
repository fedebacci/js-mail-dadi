...
function sortGuests () {
    ...
    console.debug("guestsListItems", guestsListItems);
    console.debug("guestsListItems.length", guestsListItems.length);
    const arrayToSort = [];
    console.debug("arrayToSort", arrayToSort);
    
    for (let i = 0; i <= guestsListItems.length - 1; i ++) {
        const currentListItem = guestsListItems[i];
        // NON CONTROLLO I DUPLICATI: HO GIA CONTROLLATO AL MOMENTO DELLA CREAZIONE DELLE EMAIL
        arrayToSort.push(currentListItem);
    };
