...
function sortGuests () {
    console.warn("RIORDINO")

    // TEST INIZIALE UTILIZZANDO I VALORI E CREANDO NUOVI LIST ITEM (CREDO INEFFICIENTE, VEDREI  SE POSSO RIORDINARE DIRETTAMENTE ELEMENTI HTML SENZA CANCELLARLI E RIFARLI)
    addedGuestsList.innerHTML = "";

    acceptedEmails.sort();
    console.table(acceptedEmails);
    for (let i = 0; i <= acceptedEmails.length - 1; i ++) {
        const currentEmail = acceptedEmails[i];
        console.debug("currentEmail", currentEmail);
        const newListElement = document.createElement('li');
        console.debug("newListElement", newListElement);
        newListElement.innerText = currentEmail;
        addedGuestsList.appendChild(newListElement);
    }


    // // TEST UTILIZZANDO DIRETTAMENTE I LIST ITEM
    // // ERRORE: guestsListItems.sort() is not a function
    // const guestsListItems = addedGuests.querySelectorAll('li');
    // console.debug("guestsListItems PRIMA", guestsListItems);
    // guestsListItems.sort((listElement1, listElement2) => listElement1.innerText > listElement2.innerText);
    // // guestsListItems.sort(
    // //     (listElement1, listElement2) => {
    // //         if (listElement1.innerText > listElement2.innerText) {
                
    // //         }
    // //     }
    // // );
    // console.debug("guestsListItems DOPO", guestsListItems);
